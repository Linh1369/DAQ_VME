...
// ************************************************************************
int SaveHistograms(uint32_t histo[16][4096], int numch)
{
	int i,j;
...
{
	int i, j, ch=0, chindex, wcnt, nch, pnt, ns[32], bcnt, brd_nch = 0;
	...
	uint32_t histo[16][4096];		// histograms (charge, peak or TAC)
	...
	float rate = 0.0, tm;				// trigger rate
	...
	while(!quit)  
	{
		...
		if ((CurrentTime - PrevKbTime) > 200) 
		{
			...
			if(c == 's') {
				...
				printf("Saved histograms to output file\n");
		...
		//printf("Elapsed time PC = %.2ld\n", ElapsedTime);
		if (ElapsedTime > 1000) {
			rate = (float)nev / ElapsedTime;
			ClearScreen();
			printf("Acquired %d events on channel (det) %d\n", ns[ch], ch/2);
		//	printf("Elapsed time PC %d\n", ElapsedTime);
			if (nev > 1000)
...
for (i=0; ((man->ncyc==0) || (i<man->ncyc)) && !kbhit(); i++)
{
	...
	ret = CAENVME_ReadCycle(handle,man->addr,&man->data,man->am,man->dtsize);
	if (ENABLE_LOG) 
	{
		switch (ret)
		{
			case cvSuccess   : printf(" Cycle completed normally\n");
				if((i==0) || (old_data != man->data))             
					fprintf(" Data Read : %04X",man->data & 0xFFF);
	...
	// save raw data (board memory dump)
	//if (of_raw != NULL)
			//fwrite( 		 man->data, of_raw);
	
	for(i=0; i<16; i++) 
	 	histo[i][man->data & 0xFFF]++;
